...
from tensorflow.keras.layers import concatenate, Conv2D, Conv2DTranspose, Input, MaxPool2D, Dense
...
#%%
if variant=="_gabor":
    x_train_0 = x_train_0.astype(np.int32)
    x_train_1 = x_train_1.astype(np.int32)
    x_train_2 = x_train_2.astype(np.int32)
    x_train_3 = x_train_3.astype(np.int32)
    x_test_0 = x_test_0.astype(np.int32)
    x_test_1 = x_test_1.astype(np.int32)
    x_test_2 = x_test_2.astype(np.int32)
    x_test_3 = x_test_3.astype(np.int32)
#y_train=y_train.astype(np.float32)
#y_test=y_test.astype(np.float32)

#%%
###sanity check
sample=random.randint(0, len(x_train_2))
plt.figure(figsize=(16, 8))
plt.subplot(211)
if (variant=="_gabor" or Dataset=="ISLES"):
    plt.imshow(x_train_2[sample, :,:,2], 'gray') #View each channel...
else:
    plt.imshow(x_train_2[sample, :,:], 'gray') #View each channel...
plt.subplot(212)
...
#%%
_, h, w, c = x_train_1.shape
input_shape = (h, w, c)
embed_dim=512
num_mlp=3072
numberHeads=4
transformersNumbers=4
...
with strategy.scope():
    ...
    model_1= models.transunet_2d_mhsa(input_shape, filter_num=[16, 32, 64, 128, 256], n_labels=1, stack_num_down=2, stack_num_up=2,
                                    embed_dim=embed_dim, num_mlp=num_mlp, num_heads=numberHeads, num_transformer=transformersNumbers,
                                    activation='ReLU', 
                                    mlp_activation='GELU',
                                    #mlp_activation='ReLU',
                                    ...
                                    #unpool='bilinear',
                                    unpool=True,
                                    name=('TransUnet_mhsa_1'))

    #model_1         = Dense(10, activation='relu')(model_1.output)
    #model_1=Model(inputs=model_1.input, outputs=model_1)

    #input_2=Input(shape=input_shape)
    model_2= models.transunet_2d_mhsa(input_shape, filter_num=[16, 32, 64, 128, 256], n_labels=1, stack_num_down=2, stack_num_up=2,
                                    embed_dim=embed_dim, num_mlp=num_mlp, num_heads=numberHeads, num_transformer=transformersNumbers,
                                    activation='ReLU', 
                                    mlp_activation='GELU',
                                    #mlp_activation='ReLU',
                                    ...
                                    #unpool='bilinear',
                                    unpool=True,
                                    name=('TransUnet_mhsa_2'))

#    model_2         = Dense(10, activation='relu')(model_2.output)
#    model_21=Model(inputs=model_2.input, outputs=model_2)
    
    
    #input_3=Input(shape=input_shape)
    model_3= models.transunet_2d_mhsa(input_shape, filter_num=[16, 32, 64, 128, 256], n_labels=1, stack_num_down=2, stack_num_up=2,
                                    embed_dim=embed_dim, num_mlp=num_mlp, num_heads=numberHeads, num_transformer=transformersNumbers,
                                    activation='ReLU', 
                                    mlp_activation='GELU',
                                    #mlp_activation='ReLU',
                                    ...
                                    #unpool='bilinear',
                                    unpool=True,
                                    name=('TransUnet_mhsa_3'))

#    model_3         = Dense(10, activation='relu')(model_3.output)
#    model_3=Model(inputs=model_3.input, outputs=model_3)


    #input_4=Input(shape=input_shape)
    model_4= models.transunet_2d_mhsa(input_shape, filter_num=[16, 32, 64, 128, 256], n_labels=1, stack_num_down=2, stack_num_up=2,
                                    embed_dim=embed_dim, num_mlp=num_mlp, num_heads=numberHeads, num_transformer=transformersNumbers,
                                    activation='ReLU', 
                                    mlp_activation='GELU',
                                    #mlp_activation='ReLU',
                                    ...
                                    #unpool='bilinear',
                                    unpool=True,
                                    name=('TransUnet_mhsa_4'))

#    model_4         = Dense(10, activation='relu')(model_4.output)
#    model_4=Model(inputs=model_4.input, outputs=model_4)
    

    combined=concatenate([model_1.output, model_2.output, model_3.output, model_4.output])

    z = Dense(4, activation="relu")(combined)
    z = Dense(1, activation="sigmoid")(z)

    model = Model(inputs=[model_1.input, model_2.input, model_3.input, model_4.input], outputs=z)
...
history=model.fit(x=[x_train_0, x_train_1, x_train_2, x_train_3] , y=y_train, batch_size=batchSize, 
...
y_pred =(model.predict([x_test_0, x_test_1, x_test_2, x_test_3], batch_size=batchSize)[:,:,:,0] > 0.5).astype(np.uint8)
...
#%%
test_pred_batch = (model.predict([x_test_0, x_test_1, x_test_2, x_test_3], batch_size=batchSize)[:,:,:,0] > 0.5).astype(np.uint8)
#%%
n_classes = 2
