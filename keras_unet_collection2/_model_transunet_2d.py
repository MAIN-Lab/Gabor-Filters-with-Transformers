...
from keras_unet_collection2.layer_utils2 import *
from keras_unet_collection2.activations import GELU, Snake
from keras_unet_collection2._model_unet_2d import UNET_left, UNET_right
from keras_unet_collection2.transformer_layers import patch_extract, patch_embedding
from keras_unet_collection2._backbone_zoo import backbone_zoo, bach_norm_checker

import tensorflow as tf
from tensorflow.keras.layers import Input
from tensorflow.keras.models import Model
from tensorflow.keras.layers import Layer, MultiHeadAttention, LayerNormalization, Dense, Embedding, Conv2D, Add
...
#        X_MHA=MultiHeadAttention(num_heads=num_heads, key_dim=key_dim, dropout=0.1)(X, X)
#        X_MHA = Conv2D(X.shape[-1], kernel_size=(1,1))(X_MHA)
#        X_MHA = Add()([X_MHA, X])
#        X_MHA = LayerNormalization(epsilon=1e-6)(X_MHA)
#        print(X.shape)
#        X_MHA = patch_extract((patch_size, patch_size))(X)
#        print(X_MHA.shape)
#        X_MHA = patch_embedding(num_patches, embed_dim)(X_MHA)
#        print(X_MHA.shape)
#        X_MHA = LayerNormalization(name='{}_X_MHA_layer_norm_1'.format(name))(X_MHA)
#        X_MHA = MultiHeadAttention(num_heads=num_heads, key_dim=key_dim, 
#                                 name='{}_X_MHA'.format(name))(X_MHA, X_MHA)        
#        X_MHA = tf.reshape(X_MHA, (-1, encode_size, encode_size, embed_dim))
        X_skip.append(X)

        # downsampling blocks
        for i, f in enumerate(filter_num[1:]):
            X = UNET_left(X, f, stack_num=stack_num_down, activation=activation, pool=pool, 
                          batch_norm=batch_norm, name='{}_ESTEESdown{}'.format(name, i+1))
            X_MHA = MultiHeadAttention(num_heads=num_heads, key_dim=key_dim, attention_axes=(2, 3),
                                     name='{}_X_MHA{}'.format(name, i+1))(X, X)
#            X_MHA = patch_extract((patch_size, patch_size))(X)
#            X_MHA = patch_embedding(num_patches, embed_dim)(X)
#            X_MHA = LayerNormalization(name='{}_X_MHA_layer_norm_1'.format(name))(X_MHA)
#            X_MHA = MultiHeadAttention(num_heads=num_heads, key_dim=key_dim, 
#                                     name='{}_X_MHA'.format(name))(X_MHA, X_MHA)        
#            X_MHA = tf.reshape(X_MHA, (-1, encode_size, encode_size, embed_dim))
        ...
        if depth_encode < depth_:
            ...
            for i in range(depth_-depth_encode):
                ...
                X = UNET_left(X, filter_num[i_real], stack_num=stack_num_down, activation=activation, pool=pool, 
                              batch_norm=batch_norm, name='{}_down{}_MODIFIED2'.format(name, i_real+1))
                X_skip.append(X)
...
def transunet_2d_mhsa(input_size, filter_num, n_labels, stack_num_down=2, stack_num_up=2,
                 embed_dim=768, num_mlp = 3072, num_heads=12, num_transformer=12,
